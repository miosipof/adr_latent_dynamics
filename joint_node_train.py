...
# Constants ----> YAML
EPOCHS = 20
LR = 3e-5 # x10 times lower then during NODE pre-training
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
latent_H = 4  # 4x4 => n=16
B = 32  # 1/2 of NODE pre-training batch size
...
REG = 0.01   # x10 times lower then during AE pre-training
...
def main(V=1, run=1):
    ...
    for exp_name, exp_setting in experiments.items():
        ...
        ode_func = torch.load(f"output/NODE/{exp_name}_v{V}/run_{run}/ode_last_swa.pt", map_location=DEVICE, weights_only=False)
        ...
        ode_func, losses = train_latent_dynamics(
            ...
            V=V,
            run=run,
            train_ae=True, # Unfreeze Autoencoder?
            reg_fn=reg_fn,
            lam_reg=lam_reg,
            lam_recon=1.0,
            w_anchor=1.0,
            save_dir_override=f"output/NODE_joint/{exp_name}_v{V}/run_{run}"
        )
