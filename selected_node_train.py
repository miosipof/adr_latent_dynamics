...
def main(V=1, run=1):
    ...
    for exp_name, exp_setting in experiments.items():
        ...
        enc = torch.load(f"output/AE_v{V}/{exp_name}/enc_best.pt", map_location=DEVICE, weights_only=False)
        dec = torch.load(f"output/AE_v{V}/{exp_name}/dec_best.pt", map_location=DEVICE, weights_only=False)
        ...
        ode_func, losses = train_latent_dynamics(
            ...
            lam_recon=1.0,
            w_anchor=1.0,
            save_dir_override=f"output/NODE_selected/{exp_name}_v{V}/run_{run}"
        )
